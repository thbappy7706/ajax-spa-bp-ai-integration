...
<div class="card" style="padding:14px;">
  ...
  {{-- Filters --}}
  <div class="dtc">
    <input type="text" id="ds" class="inp" placeholder="Search products…" style="width:160px;" oninput="cp=1;rT()">
    <select id="dc" class="inp" onchange="cp=1;rT()">
      <option value="">All Categories</option>
      <option>Electronics</option><option>Clothing</option><option>Software</option><option>Hardware</option><option>Books</option>
    </select>
    ...
    <button class="btn d" id="bdel" onclick="bulkDel()" style="display:none;font-size:9px;padding:3px 8px;">Delete selected</button>
  </div>
  ...
  <div style="overflow-x:auto;">
    <table id="dtt" style="min-width:640px;">
      <thead><tr>
        <th style="width:28px;"><input type="checkbox" id="ca" onchange="togAll(this)" style="accent-color:var(--ac);width:11px;height:11px;cursor:pointer;"></th>
...
<div class="mbg" id="product-modal">
  <div class="modal">
    <div class="mtitle">
      ...
      <button onclick="closeM()" style="background:none;border:none;cursor:pointer;color:var(--tm);font-size:16px;line-height:1;padding:0;">×</button>
    </div>
    <div class="frow">
      <div class="fg"><label>Product Name *</label><input type="text" class="inp" id="fn" placeholder="e.g. MacBook Pro 14"></div>
      <div class="fg"><label>SKU</label><input type="text" class="inp" id="fsk" placeholder="e.g. MBP-14"></div>
    </div>
    <div class="frow">
      <div class="fg"><label>Category *</label>
        <select class="inp" id="fct"><option value="">Select…</option><option>Electronics</option><option>Clothing</option><option>Software</option><option>Hardware</option><option>Books</option></select>
      </div>
      <div class="fg"><label>Status</label>
        <select class="inp" id="fst"><option>Active</option><option>Draft</option><option>Archived</option></select>
      </div>
    </div>
    <div class="frow">
      <div class="fg"><label>Price ($) *</label><input type="number" class="inp" id="fp" placeholder="0.00" min="0" step="0.01"></div>
      <div class="fg"><label>Stock Qty</label><input type="number" class="inp" id="fstk" placeholder="0" min="0"></div>
    </div>
    <div class="fg" style="margin-bottom:12px;">
      <label>Description</label>
      <textarea class="inp" id="fd" rows="2" placeholder="Short description…" style="resize:vertical;width:100%;"></textarea>
    </div>
...
function gS(arr) {
  return [...arr].sort((a,b) => {
    let va = a[sc], vb = b[sc];
    if (['price','stock','id'].includes(sc)) { va = +va; vb = +vb; }
...
function srtB(col) {
  if (sc === col) { sd = sd === 'asc' ? 'desc' : 'asc'; } else { sc = col; sd = 'asc'; }
...
function togRow(chk, id) { chk.checked ? sel.add(id) : sel.delete(id); updBulk(); }
function togAll(chk) {
  document.querySelectorAll('.rck').forEach(c => {
    const id = +c.dataset.id; c.checked = chk.checked;
...
function closeM() { document.getElementById('product-modal').classList.remove('open'); }
...
function saveR() {
  ...
  if (eid) { data = data.map(r => r.id === eid ? obj : r); }
  else     { data.unshift(obj); cp = 1; }
  closeM(); rT();
